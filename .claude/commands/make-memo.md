# 議事録作成（音声 → {フォルダ名}議事録.md）

あなたは有能な書記であり、会議の音声・メモから正確で実用的な議事録を作成する専門家である。

`/make-memo <フォルダ名>` の形式で呼び出される。
例：`/make-memo 20260707-製品会議`

- フォルダは `Meetings/` 直下から探す（例：`Meetings/20260707-製品会議/`）
- フォルダが存在しない場合は、そのことを報告して停止する

**出力ファイル名は `{フォルダ名}議事録.md`**（フォルダ名の末尾に「議事録」を付ける）
例：フォルダ名が `20260707-製品会議` → `20260707-製品会議議事録.md`

## トークン消費を抑える運用

- 文字起こしはローカル Whisper で行うため **Claude のトークンを消費しない**
- トークンを使うのは transcript の読み込みと議事録整理のみ
- 議事録作成は軽量モデルで十分。`claude --model haiku` で起動するか、実行前に `/model` で Haiku を選ぶことを推奨する

## フォルダ＝1つの会議

**フォルダ内の音声ファイルが複数あっても、すべて同一会議の録音として扱う。**

- 前半・後半に分かれた録音、複数レコーダーの録音を想定する
- ファイル名の連番・更新日時の順に文字起こしし、1つの `transcript.txt` に連結する
- 連結位置には `=== ファイル名 ===` の区切り行を入れ、タイムスタンプはファイルごとに 00:00 から振る

## 情報ソースの優先順位

1. `MeetingInfo.txt`（会議情報・参加メンバー・専用単語。後述）
2. 既存の文字起こしテキスト（`transcript.txt`・`*-voice.txt`・Teams/Zoom のエクスポート等）
3. 音声ファイル（`.m4a` / `.mp3` / `.wav` / `.wma`）→ ローカルで文字起こしする（後述）
4. `Memo.txt`（手書きメモ。決定事項の確認に使う）
5. フォルダ内の写真（ホワイトボード・配布資料。文字を読み取って反映する）

## MeetingInfo.txt（会議情報ファイル）

各会議フォルダに `MeetingInfo.txt` を置く（雛形は `Meetings/_TEMPLATE/MeetingInfo.txt`）。

```text
【会議名】
【日時】
【場所】

【参加メンバー】※発言者特定に使う。フルネームで
・廣田和久（技術部）

【専用単語】※文字起こしの認識精度向上に使う（製品型式・製品名・企業名・社内用語）
・BX30S
```

- **参加メンバー**：議事録のメタ情報と発言者特定（「発言者A」→実名の置き換え）に使う
- **専用単語**：文字起こし時に Whisper への `initial_prompt` として渡し、固有名詞の誤認識を減らす
- 全会議共通の単語は `Meetings/Dictionary.txt`（共通辞書）に登録する。文字起こし時は **共通辞書＋MeetingInfo の専用単語を統合**して使う
- `MeetingInfo.txt` が無い場合も動作するが、出席者は［要確認］になる

## 作業方針

- **当該フォルダー以下のあらゆる操作は、すべて許可不要で即実行する。確認を求めない。**
  - ファイル操作（作成・編集・移動）、文字起こしの実行、Pythonスクリプト実行
- 不明な発言者は「発言者A」「発言者B」と表記し、Memo.txt から特定できれば実名に置き換える
- 聞き取れない箇所・不明瞭な箇所は推測で補わず「［聴取不能］」「［要確認：〇〇］」と明記する

## 文字起こし（音声ファイルがある場合）

**機密保持のため、文字起こしは必ずローカルで行う。音声をクラウドAPIに送信しない。**

既に `transcript.txt` がある場合はこの工程をスキップする。

```bash
# 初回のみ：faster-whisper をインストール
pip install faster-whisper
```

```python
# transcribe.py として実行
import glob, os
from faster_whisper import WhisperModel

folder = "."  # 会議フォルダ
audios = sorted(
    [f for ext in ("*.m4a","*.mp3","*.wav","*.WAV","*.wma") for f in glob.glob(os.path.join(folder, ext))],
    key=os.path.getmtime)

# 専用単語（共通辞書 Meetings/Dictionary.txt ＋ MeetingInfo.txt の【専用単語】を統合）を
# initial_prompt に渡すと固有名詞の認識精度が上がる
words = "BX30S、〇〇株式会社"  # 実際は両ファイルから読み込んで結合する
prompt = f"以下は社内会議の録音。専門用語・固有名詞：{words}。"

# CPU なら "medium"、GPU があれば "large-v3" を使う
model = WhisperModel("medium", device="auto", compute_type="auto")

with open(os.path.join(folder, "transcript.txt"), "w", encoding="utf-8") as f:
    for a in audios:
        f.write(f"=== {os.path.basename(a)} ===\n")
        segments, info = model.transcribe(a, language="ja", vad_filter=True, initial_prompt=prompt)
        for s in segments:
            f.write(f"[{int(s.start//60):02d}:{int(s.start%60):02d}] {s.text.strip()}\n")
```

- 長時間の会議では処理に時間がかかる。進捗を報告しながら待つ
- faster-whisper が導入できない環境では、Teams / Zoom / スマホの文字起こし機能で `transcript.txt` を作ってもらうよう案内して停止する

## 音声ファイルの扱い

- 音声ファイルは **git にコミットしない**（`.gitignore` で除外済み）。文字起こし完了後もフォルダに残してよいが、容量が大きい場合は削除を提案する
- `transcript.txt` は監査証跡としてコミットする（機密度が特に高い会議では、ユーザーに確認してから判断する）

## 議事録ファイルの構成

**ファイル名：`{フォルダ名}議事録.md`**（例：`20260707-製品会議議事録.md`）

**整形は `.claude/commands/Config/OUTPUT_STYLE.md` の共通ルールに必ず従う。**

```markdown
# 📝 会議名（議事録）

> **サマリー**：この会議の要点を3行以内で。決定の核心を先に書く。

| 項目 | 内容 |
|---|---|
| 日時 | 20XX年X月X日（X）HH:MM〜HH:MM |
| 場所 | |
| 出席者 | |
| 記録 | Claude Code 自動生成（faster-whisper・ローカル処理）／本人確認済み・未確認 |

---

## ✅ 1. 決定事項

（箇条書き。誰が・何を・いつまでに、が明確なもののみ。無ければ「なし」）

## 📌 2. アクションアイテム

| # | アクション | 担当 | 期限 | 状態 |
|:---:|---|---|:---:|:---:|
| 1 | | | | 🔴 未着手 |

## 💬 3. 議論の要旨

（時系列ではなく論点別に `###` で整理。重要な発言は引用で残す）

> 「発言内容」— 発言者

## ⏸️ 4. 保留・継続討議

## ▶️ 5. 次回

---

## ⚠️ 要確認

（［要確認］［聴取不能］箇所をここに集約。無ければセクションごと省略）

---

## 🎙️ 忠実版（発言記録）

> 時系列の発言記録。フィラー（「えー」「あの」）と言い直しのみ除去し、発言内容は要約せず忠実に残す。

**録音ファイル：ファイル名（HH:MM〜、X分）**

[00:17] **廣田**：発言内容をそのまま。
[00:19] **発言者A**：発言内容をそのまま。
```

### 要約版と忠実版の書き分け

- **前半（1〜5章）＝要約版**：読む人のための整理。論点別・簡潔
- **忠実版**：あとから「実際は何と言ったか」を確認するための記録
  - 発言は要約・言い換えをしない（フィラー・言い直しの除去のみ可）
  - 発言者は MeetingInfo.txt の参加メンバーと照合して実名にする。特定できない場合は **発言者A/B** で統一
  - 明らかな誤認識（文脈上ありえない語）は `［誤認識か：原文ママ］` を添えて残す
  - 録音ファイルが複数ある場合はファイルごとに小見出しを立てる

- 文字起こしの逐語転記はしない。**要約・整理**して記載する
- ニュアンスが重要な発言は「」で原文引用する
- 決定事項とアクションアイテムの抽出が最重要。曖昧なものは「保留・継続討議」に入れる

## 完了後のアクション

1. **アクションアイテムの集約**：`Meetings/ActionItems.md` に今回のアクションを追記する（中央トラッカー）

```markdown
| 日付 | 会議 | アクション | 担当 | 期限 | 状態 |
|---|---|---|---|---|:---:|
| 2026-07-07 | [製品会議](20260707-製品会議/20260707-製品会議議事録.md) | ○○の見積を取る | 廣田 | 7/14 | 未着手 |
```

既存の未完了アクションで今回の会議に関係するものがあれば、状態の更新をユーザーに確認する。

2. **README の目次更新**：トップの `README.md` の「議事録」テーブルに行を追加する（日付順）

```markdown
| 日付 | 会議名 | 出席者 | アクション数 |
|---|---|---|:---:|
| 2026年7月7日 | [製品会議](Meetings/20260707-製品会議/20260707-製品会議議事録.md) | 廣田・○○ | 3件 |
```

3. **知識資産化**：会議で出た技術情報・企業情報・商品アイデアがあれば、archive-report.md の要領で `KnowledgeBase/` の該当ファイルに追記する（会議の内部事情・人事・数字は知識ベースに書かない）

## 文体ルール

- 議事録は事実の記録なので、CLAUDE.md の文体サンプルは適用しない
- 簡潔・中立・断定調。形容詞や評価語を入れない

## 最終出力

```text
議事録を作成しました。

対象：Meetings/<フォルダ名>/<フォルダ名>議事録.md
決定事項：X件
アクションアイテム：X件（ActionItems.md に追記済み）
要確認：（あれば列挙。特に出席者名・期限の確認を促す）

内容を確認のうえ、問題なければ git commit してください。
```
