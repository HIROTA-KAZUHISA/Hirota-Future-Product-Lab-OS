...
2. **会議音声**（録音 or 文字起こし）→ Gijiroku.md → アクションアイテム集約
...
| `Meetings/` | 議事録（`YYYYMMDD-会議名/Gijiroku.md` 形式） |
...
2. `/make-minutes YYYYMMDD-会議名` を実行
3. Gijiroku.md 生成 → ActionItems.md へアクション集約 → README 目次更新
